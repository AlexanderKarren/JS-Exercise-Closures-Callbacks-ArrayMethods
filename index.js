...
function processDuplicateFree(list) {
  const deDupedSet = new Set(list);
  const deDupedList = [...deDupedSet];
  return deDupedList;
}
// working
...
function firstNamesAllCaps(runners) {
  const upperCaseRunners = runners.map(function(runner) {
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const specifiedRunners = [];
  runners.forEach(runner => {
    if (runner.shirt_size === tShirtSize) {
      specifiedRunners.push(runner);
    }
  });
  return specifiedRunners;
}
// working
...
function tallyUpDonations(runners) {
  const donationSum = runners.reduce(function(accumulator, runner) {
